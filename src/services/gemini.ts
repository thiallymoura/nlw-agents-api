...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
    ...
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: "Transcreva o áudio para oportuguês do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em páragrafos quando for apropriado"
            },
            {
                inlineData: {
                    mimeType,
    ...
    if (!response.text) {
        throw new Error('Não foi póssivel transcrever o áudio')
    }
